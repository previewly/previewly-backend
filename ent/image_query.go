...
func (iq *ImageQuery) QueryImageprocess() *ImageProcessQuery {
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withImageprocess: iq.withImageprocess.Clone(),
...
func (iq *ImageQuery) WithImageprocess(opts ...func(*ImageProcessQuery)) *ImageQuery {
	query := (&ImageProcessClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withImageprocess = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withImageprocess; query != nil {
		if err := iq.loadImageprocess(ctx, query, nodes,
			func(n *Image) { n.Edges.Imageprocess = []*ImageProcess{} },
			func(n *Image, e *ImageProcess) { n.Edges.Imageprocess = append(n.Edges.Imageprocess, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadImageprocess(ctx context.Context, query *ImageProcessQuery, nodes []*Image, init func(*Image), assign func(*Image, *ImageProcess)) error {
	...
	query.Where(predicate.ImageProcess(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.ImageprocessColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.image_imageprocess
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_imageprocess" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_imageprocess" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
